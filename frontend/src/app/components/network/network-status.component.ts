...
import { BackgroundFade, NetworkBackendStatusMessage, NetworkGuardPopup, NetworkStatusPopup } from 'components/network/network-status.animations';
...
@Component({
  ...
  animations:      [ NetworkBackendStatusMessage, BackgroundFade, NetworkStatusPopup, NetworkGuardPopup ]
})
export class NetworkStatusComponent {
  ...
  public readonly isBackendDead$ = this.store.pipe(select(fromRoot.isNetworkBackendDead));
  ...
  public hideOfflineWarning(): void {
    this.store.dispatch(NetworkActions.GoOnline({ pingBackend: false }));
    this.store.dispatch(NetworkActions.pingBackendScheduleStop());
  }
}
